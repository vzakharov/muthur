...
same stack" is an answer; so is *"this repo, but in Python"*.
...
is named after too, so it travels as a copy rather than a rename. Clear both of
its stub markers — the banner and the `STUB` in its frontmatter description. The
watermark you just wrote *is* the hydration, and
...
satisfying assertion 1 *is* "enough to run the loop".
...
*rewrites*, the content that actually wants reading, rather than a hundred files
