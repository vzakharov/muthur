...
- **The alternatives land far apart.** Git reverts anything on a branch, so reversibility is not the measure. Distance is: build the plan's choice, then picture the operator reading the diff and asking for the next-best option, and count what that switch rewrites. A default value is a line apart; a data shape every later file reads is the whole branch apart, and grows with each commit built on it before the PR is read. A step off the branch — a filed issue, a posted comment, a deploy — has no diff to revert, so it counts as farthest of all.
