...
| `.claude/hooks/prompt-issue-export.sh` | On every prompt, export each `#<N>` or thread URL it carries — at most three, skipping what is already on disk — so `/take-issue` Step 1 is done before the agent reads the prompt. Commits nothing, and reports a failure rather than raising one. `SessionStart` cannot carry this: it fires before the prompt exists. | `bash`, `jq`, `python3` ≥3.9, `scripts/export-github-item.py`, `.claude/settings.json` wiring (G4) | `/take-issue` | adopt |
