...
| `.claude/rules/` | The path-scoped convention mechanism: a rule file loads only when a session touches the paths it declares. Ships with a README and the loop's own three rules: `stack.md` (what `vet.sh` exits, what a stack landing wires), `skills.md` (adding or renaming a skill) and `staging.md` (editing a file that loads on every turn through a staged copy). | — | `scripts/staged.sh`, `/finalize` (G2), for `staging.md` — without them, drop it and the line in `skills.md` that points at it | adopt |
...
| `.claude/hooks/file-tools-nudge.py` | Before each `Bash` call, refuse the first command that reads a file (`cat`, `head`, `sed -n` …), edits one in place (`sed -i`, `perl -pi` …) or writes one (a heredoc or `echo` into it, `tee`), naming the tool `CLAUDE.md` asks for instead; the identical command again in the same session goes through, so a deliberate mass substitution costs one retry. Records what it refused under `tmp/`, and fails open. | `python3` ≥3.9, `.claude/settings.json` wiring (G4) | — | adopt |
...
| `/plan` | Write the plan to a `docs/plans/` file whose name is the approval gate, publish it as a draft PR so it is reviewed as a diff, and ask questions as numbered prose. Also owns the call on whether work is beyond one session, and which shape splits it: an elephant, a bite per session in one PR, or a pizza, slices carved into issues. Carries `elephant.md`, the megaplan's shape and how a bite is taken; `carving.md`, what a pizza's plan names as its proposed parent and children and what `/go` files from it; and `native-plan-mode.md`, the recovery for a session that reached native plan mode anyway, which is plan mode's own exit rather than an override of it. | `gh` | `/finalize`, `/go`, `/pr`; **conditionally** `/take-issue`, `/propose-issue` (G3) | adopt |
...
| `/finalize` | Land prep: swap in staged copies, the quality passes, vet, merge the base, sweep working artifacts, flip to ready, reconcile the squash message, attest — and, on `and merge`, merge the PR when the run turned up nothing to decide. | `gh`, `scripts/vet.sh`, `scripts/staged.sh` | `/polish` (G1); `/check-merge`, `/from-branch`, `/plan`, `/squash-message`; **conditionally** `/take-issue` (G3), `/watch-ci` (G5) | adopt |
...
| `/relay` | Hand the session to a fresh one instead of compacting it: write a summary to a committed `docs/remove-before-merging/relay.md`, start a successor on the branch with `/relay take <branch>`, and stop. The successor attaches, reads the summary, and dispatches on its Next step. | `create_session` for the spawn on the web; a typed line locally | `/from-branch`, `/go`, `/finalize` | adopt |
...
| `scripts/staged.sh` | Stage an always-loaded file as a byte-identical copy at `.claude/staged/<path>.staged`, and swap it back at `/finalize` — merging in whatever the real file gained meanwhile. Also the check the vet run makes, and the lookup `check-skill-catalog.sh` cites through. | `bash`, `git` | — | adopt |
| `scripts/vet.sh` | The vet run: the fast lint/type-check/test pass before pushing review-ready work. | your stack's own commands | `scripts/check-skill-catalog.sh` (G1), `scripts/check-squash-message.sh`, `scripts/staged.sh`, `scripts/check-muthur.sh` (never) | **rewrite** |
