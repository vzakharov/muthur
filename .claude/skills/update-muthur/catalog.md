...
| `.claude/hooks/prompt-issue-export.sh` | Export the `#<N>` or thread URL a prompt *ends* in — skipping what is already on disk — so `/take-issue` Step 1 is done before the agent reads the prompt. Costs one `jq` on a prompt that ends in anything else. Commits nothing, and reports a failure rather than raising one. | `bash`, `jq`, `python3` ≥3.9, `scripts/export-github-item.py`, `.claude/settings.json` wiring (G4) | `/take-issue` | adopt |
