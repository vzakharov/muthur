...
| `scripts/staged.sh` | Stage an always-loaded file as a byte-identical copy under `docs/remove-before-merging/`, recorded in a manifest, and swap it back at `/finalize` — merging in whatever the real file gained meanwhile. Also the manifest check the vet run makes, and the lookup `check-skill-catalog.sh` cites through. | `bash`, `git` | — | adopt |
