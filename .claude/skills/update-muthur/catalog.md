...
| `scripts/staged.sh` | Stage an always-loaded file as a byte-identical copy at the mirrored path under `docs/staged/`, and swap it back at `/finalize` — merging in whatever the real file gained meanwhile. Also the check the vet run makes, and the lookup `check-skill-catalog.sh` cites through. The `claudeMdExcludes` entry in `.claude/settings.json` goes with it. | `bash`, `git` | — | adopt |
