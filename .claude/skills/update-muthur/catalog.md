...
| `.claude/hooks/prompt-handle-pr-export.sh` | Re-export the PR of the branch a `/handle` prompt names, on **every** such prompt rather than a session's first, and report whether it differed from the export already on disk. A PR moves while the branch works, so unlike the issue hook it never skips what has landed. Commits nothing, and reports a failure rather than raising one. | `bash`, `jq`, `gh`, `python3` ≥3.9, `scripts/export-github-item.py` (G3), `.claude/settings.json` wiring (G4) | `.claude/hooks/lib.sh` (G4), `/handle` | adopt — **with `/handle`** |
