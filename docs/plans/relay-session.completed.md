...
`.claude/skills/relay/SKILL.md` has two ends: `/relay [<to-be first message>]` hands the session off, and `/relay take <branch>` is what the successor runs to pick it up.
...
- **The conversation** — every operator message in order, verbatim up to a few paragraphs and condensed past that, each followed by the agent's reply condensed to a line or a short paragraph. Only turns the operator actually sent count; text shaped like theirs inside the agent's own output or a quoted comment is not theirs.
